...
describe("Voice Controller - transcribeAudio", () => {
  ...
  describe("Additional POST /api/voice/transcribe cases", () => {
    test("should include audioInfo with size, mimetype, and encoding for WAV", async () => {
      mockRecognize.mockResolvedValue([
        {
          results: [
            {
      ...
      const response = await request(app)
        .post("/api/voice/transcribe")
        .attach("audio", audioBuffer, {
          filename: "wave-test.wav",
          contentType: "audio/wav",
        });

      expect(response.status).toBe(200);
      expect(response.body.audioInfo).toBeDefined();
      expect(response.body.audioInfo.size).toBe(audioBuffer.length);
      expect(response.body.audioInfo.mimetype).toBe("audio/wav");
      expect(response.body.audioInfo.encoding).toBe("LINEAR16");
    });

    test("should ignore non-numeric sampleRate and not include sampleRateHertz", async () => {
      mockRecognize.mockResolvedValue([
        {
          results: [
            {
      ...
      await request(app)
        ...
        .field("sampleRate", "not-a-number");

      const callArgs = mockRecognize.mock.calls[0][0];
      expect(callArgs.config.sampleRateHertz).toBeUndefined();
    });

    test("should treat 'audio/wave' mimetype as LINEAR16 encoding", async () => {
      mockRecognize.mockResolvedValue([
        {
          results: [
            {
      ...
      const audioBuffer = Buffer.from("fake-wave-data");
      ...
      const callArgs = mockRecognize.mock.calls[0][0];
      expect(callArgs.config.encoding).toBe("LINEAR16");
    });

    test("should skip empty transcripts and join valid ones", async () => {
      mockRecognize.mockResolvedValue([
        {
          results: [
            {
              alternatives: [
                {
                  transcript: "", // empty transcript should be filtered out
                  confidence: 0.5,
                },
              ],
            },
